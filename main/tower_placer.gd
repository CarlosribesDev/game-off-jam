...
func _process(_delta: float) -> void:
	...
	var map_coords = level_tile_map.local_to_map(level_tile_map.to_local(mouse_pos))
	var tile_data = level_tile_map.get_cell_tile_data(map_coords)
	...
	if tile_data != null:
		...
		if tile_data.get_custom_data("buildeable"):
			...
			# get_tile_pos 
			var center_pos_local = level_tile_map.map_to_local(map_coords)
			
			
			_snapped_world_position = level_tile_map.to_global(center_pos_local)
...
func _input(event: InputEvent) -> void:
	...
	if _is_placing and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		if _is_valid_placement:
			...
			#var map_coords = level_tile_map.local_to_map(level_tile_map.to_local(get_global_mouse_position()))
